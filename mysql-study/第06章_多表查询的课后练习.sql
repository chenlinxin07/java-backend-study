...
FROM employees e LEFT OUTER JOIN departments d  #保左表,左表全显示，配不上就空着
...
# 6.选择指定员工的姓名，员工号，以及他的管理者的姓名和员工号，结果类似于下面的格式(起别名)
...
SELECT e.employee_id,e.last_name,e.department_id,d.department_name "部门名"
FROM employees e JOIN departments d
ON e.`department_id` = d.`department_id`
